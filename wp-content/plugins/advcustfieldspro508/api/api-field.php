...
function acf_get_field( $selector = null ) {
	...
	if( $k == 'ID' )
	{
		$field = _acf_get_field_by_id( $v );
               // error_log("2 acf_get_field field: ".$field);
	}
	elseif( $k == 'name' )
	{
		$field = _acf_get_field_by_name( $v );
               // error_log("1 acf_get_field field: ".$field);
	}
	else
	{
		$field = _acf_get_field_by_key( $v );
                //error_log("3 acf_get_field field: ".$field);
	}
...
function _acf_get_field_by_key( $key = '' ) {
	...
	if( acf_is_local_field( $key ) )
	{
	 ...
	 //error_log("acf_get_field_by_key: ".print_r($key,true));
	// vars
         //global $post;
	$args = array(
		'posts_per_page'	=> 1,
	...
	if( empty($posts) )
	{
          // error_log("4 return empty: ".print_r($post,true));
		return $field;	
	}
	
	//error_log("5 return empty: ".$posts[0]->ID);
	// load from ID
	$field = _acf_get_field_by_id( $posts[0]->ID );
	//$field = _acf_get_field_by_id( 2324 );
