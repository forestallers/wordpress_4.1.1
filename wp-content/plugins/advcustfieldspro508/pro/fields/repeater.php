	...
	<tbody>
		<?php foreach( $field['value'] as $i => $row ): ?>
			<tr class="acf-row<?php echo ($i === 'acfcloneindex') ? ' clone' : ''; ?>">
				...
				<?php if( $show_order ): ?>
					<td class="order" style="vertical-align: top" title="<?php _e('Drag to reorder','acf'); ?>"><?php echo intval($i) + 1; ?></td>
				<?php endif; ?>
				...
				<?php if( $show_remove ): ?>
					<td class="remove" style="vertical-align: top">
						<a class="acf-icon small acf-repeater-add-row" href="#" data-before="1" style="display:none" title="<?php _e('Add row','acf'); ?>"><i class="acf-sprite-add"></i></a>
						<a class="acf-icon small acf-repeater-remove-row" href="#" title="<?php _e('Remove row','acf'); ?>"><i class="fa fa-minus-circle fa-lg"></i></a>
					</td>
				<?php endif; ?>
